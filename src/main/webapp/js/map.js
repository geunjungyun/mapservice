...
//var server = "http://192.168.100.209:8070/ver1/mapservice";

var server = "./ver1/mapservice";

//var key = "abcd1234efgh5678ijkl91011mnop1213qrst1415uvwx";
...
function constructSourceEmap(tileset, tile) {
	...
	var source = new ol.source.XYZ({
		...
		tileUrlFunction: function(coordinate) {
			...
			url = server + "?req=tile&path="+tileset+"/"+tile+"/" + z + "/" + x + '/' + y + '.png'+"&key="+key;
			return url;
		}
	});
...
function getTileList(){
	...
	var	html ='<button style="margin-top:5px; height:25px; width: 200px;" onclick="reloadtile()">타일리로드</button><br>';
...
function logCenterAndZoom() {
    const center = view.getCenter(); // 중심 좌표를 경위도로 변환
