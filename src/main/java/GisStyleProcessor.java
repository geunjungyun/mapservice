 ...
 * java -Dfile.encoding=UTF-8 GisStyleProcessor [styIn] [mapIn] [styOut]
 * [mapOut]
 */
public class GisStyleProcessor {
    ...
    static {
        ...
        // ❌ 제외: tn_ex_hwrdcenl_l, tn_ex_cityhwrdcenl_l, tn_ex_spa_a, tn_ex_clrtroute_l
        // (사이레이어 존재)
    ...
    static {
        COLOR_KR.put("202/199/167", "황갈");
        COLOR_KR.put("199/197/162", "연황갈");
        COLOR_KR.put("193/191/153", "회황");
        COLOR_KR.put("179/177/145", "회녹");
        COLOR_KR.put("155/155/125", "회");
        COLOR_KR.put("137/137/105", "암회");
        COLOR_KR.put("207/201/158", "연황");
        COLOR_KR.put("194/188/148", "황녹");
        COLOR_KR.put("174/168/130", "회갈");
        COLOR_KR.put("157/151/115", "진회");
        COLOR_KR.put("210/210/165", "연녹");
        COLOR_KR.put("185/185/140", "녹회");
    }
    ...
    private static class LayerEntry {
        ...
        double drawOrder;
        Element layerInfoElem;
    }

    private static class LeafQuery {
        String cql, field, value, leafStyle;
    }

    private static class LvInfo {
        String styleName, full;
        LeafQuery lq;
        String vh;
        Element li;
        double draw;
    ...
    public static void main(String[] args) throws Exception {
        ...
        // args[4]: 쉼표로 구분된 mapInfo 주제도명 목록 (예: "emp01,emp02").
        // 지정하면 mo 파일명을 기준으로 mapInfo_{name}.xml 여러 개를 각각 <name>도 다르게 저장한다.
        String[] mapThemes = args.length > 4 ? args[4].split(",") : null;
        new GisStyleProcessor().process(si, mi, so, mo, mapThemes);
    }

    public void process(String si, String mi, String so, String mo) throws Exception {
        process(si, mi, so, mo, null);
    }

    public void process(String si, String mi, String so, String mo, String[] mapThemes) throws Exception {
        DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();
        dbf.setFeature("http://apache.org/xml/features/nonvalidating/load-external-dtd", false);
        DocumentBuilder db = dbf.newDocumentBuilder();
        styDoc = db.parse(new File(si));
        mapDoc = db.parse(new File(mi));
        buildStyleMap();
        out.printf("[로드] styles:%d / layerInfo:%d%n",
                styleMap.size(), mapDoc.getElementsByTagName("layerInfo").getLength());

        int[] sr = applySubwayRename();
        out.printf("[지하철명칭] name=%d cql=%d styleName=%d value=%d%n",
                sr[0], sr[1], sr[2], sr[3]);

        replaceExitNumberStyle();
        out.printf("[출구번호스타일] tn_ex_exitnumber_p+지하철출구번호 교체%n");

        int subLbl = applySubwayLabelOverrides();
        out.printf("[지하철역명라벨오버라이드] extensionArea=0, lineOverflow=true 적용 %d건%n", subLbl);

        int snt = applySymbolNameTrim();
        out.printf("[symbolName정리] %d건%n", snt);

        int cr = applyCqlRewrite();
        out.printf("[CQL재작성] poi_code=1 → sym_name_s='고속도로번호': %d건%n", cr);

        int cl = applyCqlColumnLower();
        out.printf("[CQL컬럼소문자] %d건%n", cl);

        int[] tf = applyTextStyleFont();
        out.printf("[textStyle폰트] fontName추가=%d fontType추가=%d%n", tf[0], tf[1]);

        int[] rl = removeLayerStyles(new String[] {
                "walk_area", "service_area", "walk_line" });
        out.printf("[레이어삭제] styles=%d mapInfo=%d%n", rl[0], rl[1]);

        int rl2 = removeMapInfoLayers(new String[] { "tn_ex_subwsta_l_conv" });
        out.printf("[mapInfo레이어삭제] %d건 (스타일 유지)%n", rl2);
        ...
        out.printf("[완료] 스타일:%d / layerInfo:%d%n",
                styleMap.size(), mapDoc.getElementsByTagName("layerInfo").getLength());
        removeViewLayers(); // _view 레이어 및 연관 스타일 제거

        int ss = syncSelectStyleNames();
        out.printf("[selectStyleName동기화] ~ → - 변환: %d건%n", ss);

        int sfn = applySymbolFieldNameOverrides();
        out.printf("[symbolStyle fieldName] %d건%n", sfn);

        int subLine = fixSubwayLineNameDrawOrderLast();
        out.printf("[지하철노선명drawOrder고정] %d건%n", subLine);

        saveXml(styDoc, so);
        if (mapThemes == null || mapThemes.length == 0) {
            saveXml(mapDoc, mo);
        } else {
            Element mapRoot = mapDoc.getDocumentElement();
            for (String theme : mapThemes) {
                theme = theme.trim();
                if (theme.isEmpty())
                    continue;
                setChildText(mapRoot, "name", theme);
                saveXml(mapDoc, themeOutPath(mo, theme));
            }
        }
    }

    // mo=".../mapInfo.xml", theme="emp01" → ".../mapInfo_emp01.xml"
    private static String themeOutPath(String mo, String theme) {
        int slash = Math.max(mo.lastIndexOf('/'), mo.lastIndexOf('\\'));
        int dot = mo.lastIndexOf('.');
        if (dot < 0 || dot < slash)
            return mo + "_" + theme;
        return mo.substring(0, dot) + "_" + theme + mo.substring(dot);
    }
    ...
    // ══════════════════════════════════════════════════════════
    // STEP 0: 지하철 노선명 변경
    // 서울지하철N호선 → 수도권N호선 (긴 패턴 먼저)
    // 서울N호선 → 수도권N호선
    // 적용 태그: <name>, <cql>, <styleName>, <value>
    // ══════════════════════════════════════════════════════════
    private int[] applySubwayRename() {
        // 변환 규칙 (긴 패턴 먼저 적용하여 이중 치환 방지)
        String[][] rules = new String[18][2];
        for (int n = 1; n <= 9; n++) {
            rules[(n - 1) * 2] = new String[] { "서울지하철" + n + "호선", "수도권" + n + "호선" };
            rules[(n - 1) * 2 + 1] = new String[] { "서울" + n + "호선", "수도권" + n + "호선" };
        }

        int[] cnt = { 0, 0, 0, 0 }; // name, cql, styleName, value
        String[] tagList = { "name", "cql", "styleName", "value" };
        int[] tagIndex = { 0, 1, 2, 3 };

        for (int ti = 0; ti < tagList.length; ti++) {
            String tag = tagList[ti];
            NodeList nodes = styDoc.getElementsByTagName(tag);
            for (int i = 0; i < nodes.getLength(); i++) {
                org.w3c.dom.Node node = nodes.item(i);
                String text = node.getTextContent();
                String replaced = text;
                // 규칙 1: 서울지하철N호선, 서울N호선 → 수도권N호선
                for (String[] rule : rules)
                    replaced = replaced.replace(rule[0], rule[1]);
                // 규칙 2: 단독 N호선 (앞에 한글 없는) → 수도권N호선
                // 예: '9호선', +9호선 → 단, 부산1호선, 수도권1호선 등은 유지
    ...
    // ══════════════════════════════════════════════════════════
    // STEP 0-2: CQL 컬럼명 및 fieldName 대문자 → 소문자
    // CQL 대상: LINE_NAME, NAME, S_LEVEL, CLIM_CD, TYPE,
    // RLROAD_SE, RLWTY_SE, BLDSL_NO
    // fieldName: symbolStyle/textStyle 하위 fieldName 전체
    // ══════════════════════════════════════════════════════════
    private int applyCqlColumnLower() {
        String[] upperCols = {
                "LINE_NAME", "NAME", "S_LEVEL", "CLIM_CD",
                "TYPE", "RLROAD_SE", "RLWTY_SE", "BLDSL_NO"
        };
        int cnt = 0;

        // <cql> 컬럼명 소문자화
        NodeList cqlNodes = styDoc.getElementsByTagName("cql");
        for (int i = 0; i < cqlNodes.getLength(); i++) {
            org.w3c.dom.Node node = cqlNodes.item(i);
            String text = node.getTextContent();
            String replaced = text;
            for (String col : upperCols)
                replaced = replaced.replaceAll("\\b" + col + "\\b", col.toLowerCase());
            if (!replaced.equals(text)) {
                node.setTextContent(replaced);
                cnt++;
            }
        ...
        // <fieldName> 소문자화 (symbolStyle/textStyle 하위)
        NodeList fnNodes = styDoc.getElementsByTagName("fieldName");
        for (int i = 0; i < fnNodes.getLength(); i++) {
            org.w3c.dom.Node node = fnNodes.item(i);
            String text = node.getTextContent();
    ...
    // ══════════════════════════════════════════════════════════
    // STEP 0-3: textStyle 폰트 설정 추가
    // · <fontName>NanumGothicBold.ttf</fontName> 전체 추가
    // · <fontType>BOLD</fontType> 없을 경우 추가
    // 삽입 위치: multiLineSeperator 다음 (없으면 첫 자식 앞)
    // ══════════════════════════════════════════════════════════
    private int[] applyTextStyleFont() {
        int addedName = 0, addedType = 0;
        NodeList tsList = styDoc.getElementsByTagName("textStyle");
        for (int i = 0; i < tsList.getLength(); i++) {
            Element ts = (Element) tsList.item(i);

            // 삽입 기준점: multiLineSeperator 다음 or 첫 자식
            org.w3c.dom.Node anchor = null;
            NodeList children = ts.getChildNodes();
            for (int j = 0; j < children.getLength(); j++) {
                org.w3c.dom.Node n = children.item(j);
                if (n.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE
                        && "multiLineSeperator".equals(n.getNodeName())) {
                    anchor = n.getNextSibling();
                    break;
                }
            }
            // anchor == null 이면 첫 번째 요소 앞
            if (anchor == null) {
                for (int j = 0; j < children.getLength(); j++) {
                    if (children.item(j).getNodeType() == org.w3c.dom.Node.ELEMENT_NODE) {
                        anchor = children.item(j);
                        break;
                    }
                }
            }
            ...
            // <fontType> 없으면 추가
            if (ts.getElementsByTagName("fontType").getLength() == 0) {
                Element ft = styDoc.createElement("fontType");
                ft.setTextContent("BOLD");
                ts.insertBefore(ft, anchor);
                if (anchor != null)
                    anchor = ft.getNextSibling();
                addedType++;
            }

            // <fontName> 항상 추가 (fontType 뒤)
            if (ts.getElementsByTagName("fontName").getLength() == 0) {
                // fontType 다음으로 anchor 재조정
                org.w3c.dom.Node ftNode = ts.getElementsByTagName("fontType").item(0);
                org.w3c.dom.Node fnAnchor = ftNode != null ? ftNode.getNextSibling() : anchor;
                Element fn = styDoc.createElement("fontName");
                fn.setTextContent("NanumGothicBold.ttf");
                ts.insertBefore(fn, fnAnchor);
                addedName++;
            }
        }
        return new int[] { addedName, addedType };
    }

    // ══════════════════════════════════════════════════════════
    // STEP 0-4: 특정 레이어 연결 스타일 삭제
    // 1) mapInfo의 해당 레이어 layerInfo 제거
    // 2) 해당 레이어가 참조한 스타일 styles.xml에서 제거
    // ══════════════════════════════════════════════════════════
    // ══════════════════════════════════════════════════════════
    // mapInfo.xml 레이어만 삭제 (styles.xml 스타일은 유지)
    // ══════════════════════════════════════════════════════════
    private int removeMapInfoLayers(String[] layers) {
        java.util.Set<String> layerSet = new java.util.HashSet<>(java.util.Arrays.asList(layers));
        int cnt = 0;
        NodeList liList = mapDoc.getElementsByTagName("layerInfo");
        ...
        for (int i = 0; i < liList.getLength(); i++) {
            ...
            NodeList lns = li.getElementsByTagName("layerName");
            if (lns.getLength() == 0)
                continue;
            String bare = lns.item(0).getTextContent().trim();
            if (bare.contains(":"))
                bare = bare.substring(bare.lastIndexOf(':') + 1);
            if (layerSet.contains(bare))
                toRemove.add(li);
        }
    ...
    private int[] removeLayerStyles(String[] layers) {
        java.util.Set<String> layerSet = new java.util.HashSet<>(java.util.Arrays.asList(layers));
        java.util.Set<String> targetStyles = new java.util.LinkedHashSet<>();
        int removedMap = 0, removedSty = 0;

        // 1) mapInfo: 대상 레이어 layerInfo 수집 및 제거
        NodeList liList = mapDoc.getElementsByTagName("layerInfo");
        java.util.List<org.w3c.dom.Node> toRemoveMap = new java.util.ArrayList<>();
        for (int i = 0; i < liList.getLength(); i++) {
            Element li = (Element) liList.item(i);
            NodeList lns = li.getElementsByTagName("layerName");
            if (lns.getLength() == 0)
                continue;
            String layerName = lns.item(0).getTextContent().trim();
            String bare = layerName.contains(":") ? layerName.substring(layerName.lastIndexOf(':') + 1) : layerName;
            if (layerSet.contains(bare)) {
                NodeList sns = li.getElementsByTagName("selectStyleName");
                if (sns.getLength() > 0)
                    targetStyles.add(sns.item(0).getTextContent().trim());
                toRemoveMap.add(li);
            }
        }
        for (org.w3c.dom.Node n : toRemoveMap) {
            n.getParentNode().removeChild(n);
            removedMap++;
        }

        // 2) styles.xml: 수집된 스타일 제거
        for (String tag : new String[] { "polygonStyle", "polylineStyle", "pointStyle" }) {
            NodeList slist = styDoc.getElementsByTagName(tag);
            java.util.List<org.w3c.dom.Node> toRemoveSty = new java.util.ArrayList<>();
            for (int i = 0; i < slist.getLength(); i++) {
                Element e = (Element) slist.item(i);
                NodeList names = e.getElementsByTagName("name");
                if (names.getLength() > 0 &&
                        targetStyles.contains(names.item(0).getTextContent().trim()))
                    toRemoveSty.add(e);
            }
            for (org.w3c.dom.Node n : toRemoveSty) {
                n.getParentNode().removeChild(n);
                removedSty++;
            }
        }
        return new int[] { removedSty, removedMap };
    }

    // ══════════════════════════════════════════════════════════
    // tn_ex_exitnumber_p+지하철출구번호 스타일 전체 교체
    // ══════════════════════════════════════════════════════════
    // ══════════════════════════════════════════════════════════
    // CQL 조건 재작성
    // poi_code=1 → sym_name_s='고속도로번호'
    // 원본: <name>poi_code</name> + <value>1</value>
    // 변경: <name>sym_name_s</name> + <value>고속도로번호</value>
    // ══════════════════════════════════════════════════════════
    private int applyCqlRewrite() {
        int cnt = 0;
        NodeList queries = styDoc.getElementsByTagName("query");
        for (int i = 0; i < queries.getLength(); i++) {
            Element q = (Element) queries.item(i);
            NodeList nameNodes = q.getElementsByTagName("name");
            NodeList valueNodes = q.getElementsByTagName("value");
            if (nameNodes.getLength() == 0 || valueNodes.getLength() == 0)
                continue;
            org.w3c.dom.Node nameNode = nameNodes.item(0);
            org.w3c.dom.Node valueNode = valueNodes.item(0);
            if ("poi_code".equals(nameNode.getTextContent().trim())
                    && "1".equals(valueNode.getTextContent().trim())) {
                nameNode.setTextContent("sym_name_s");
                valueNode.setTextContent("\uace0\uc18d\ub3c4\ub85c\ubc88\ud638");
                cnt++;
            }
        }
        // 이미 <cql>poi_code=1</cql> 형태로 존재하는 경우도 처리
        NodeList cqlNodes = styDoc.getElementsByTagName("cql");
        for (int i = 0; i < cqlNodes.getLength(); i++) {
            org.w3c.dom.Node node = cqlNodes.item(i);
            String text = node.getTextContent();
            String replaced = text
                    .replace("poi_code=1", "sym_name_s='\uace0\uc18d\ub3c4\ub85c\ubc88\ud638'")
                    .replace("poi_code = 1", "sym_name_s='\uace0\uc18d\ub3c4\ub85c\ubc88\ud638'");
            if (!replaced.equals(text)) {
                node.setTextContent(replaced);
                cnt++;
    ...
    // ══════════════════════════════════════════════════════════
    // symbolName 정리: 첫 번째 '_' 이전 접두어 제거
    // 예: 교통_지하철환승.png → 지하철환승.png
    // 산악_기점.png → 기점.png
    // (언더바 없으면 유지)
    // ══════════════════════════════════════════════════════════
    private int applySymbolNameTrim() {
        int cnt = 0;
        NodeList nodes = styDoc.getElementsByTagName("symbolName");
        for (int i = 0; i < nodes.getLength(); i++) {
            org.w3c.dom.Node node = nodes.item(i);
            String text = node.getTextContent().trim();
            int idx = text.indexOf('_');
            if (idx > 0) {
                node.setTextContent(text.substring(idx + 1));
                cnt++;
            }
        }
        return cnt;
    ...
    private void replaceExitNumberStyle() {
        final String TARGET = "tn_ex_exitnumber_p+\uc9c0\ud558\ucca0\ucd9c\uad6c\ubc88\ud638";
        NodeList psList = styDoc.getElementsByTagName("pointStyle");
        for (int i = 0; i < psList.getLength(); i++) {
            Element ps = (Element) psList.item(i);
            NodeList names = ps.getElementsByTagName("name");
            if (names.getLength() == 0)
                continue;
            if (!TARGET.equals(names.item(0).getTextContent().trim()))
                continue;

            // 기존 자식 노드 전체 제거
            while (ps.hasChildNodes())
                ps.removeChild(ps.getFirstChild());

            // 새 구조 생성
            addText(styDoc, ps, "name", TARGET);
            addText(styDoc, ps, "positionType", "xcx");
            addText(styDoc, ps, "priority", "0");

            // symbolStyle
            Element sym = styDoc.createElement("symbolStyle");
            addText(styDoc, sym, "fieldName", "\uc9c0\ud558\ucca0\uc5ed\ucd9c\uad6c\ubc88\ud638_{name}.png");
            addText(styDoc, sym, "width", "12");
            addText(styDoc, sym, "height", "12");
            ps.appendChild(sym);

            addText(styDoc, ps, "transparency", "0");

            // overlapConfig
            Element oc = styDoc.createElement("overlapConfig");
            addText(styDoc, oc, "extensionArea", "0");
            addText(styDoc, oc, "deleteOverlapMode", "NotDelete");
            ps.appendChild(oc);
            return;
        }
        out.printf("[출구번호스타일] 대상 스타일 미발견%n");
    }

    // "지하철역명11레벨+LINE_NAME=..." 전체 노선 라벨 스타일에
    // overlapConfig.extensionArea=0, textStyle.lineOverflow=true 일괄 적용
    private int applySubwayLabelOverrides() {
        final String PREFIX = "지하철역명11레벨+LINE_NAME";
        int cnt = 0;
        NodeList psList = styDoc.getElementsByTagName("pointStyle");
        for (int i = 0; i < psList.getLength(); i++) {
            Element ps = (Element) psList.item(i);
            String name = getChildText(ps, "name");
            if (name == null || !name.startsWith(PREFIX))
                continue;

            Element oc = getFirstChild(ps, "overlapConfig");
            if (oc == null) {
                oc = styDoc.createElement("overlapConfig");
                ps.appendChild(oc);
            }
            if (getFirstChild(oc, "extensionArea") == null) {
                addText(styDoc, oc, "extensionArea", "0");
            } else {
                setChildText(oc, "extensionArea", "0");
            }

            Element ts = getFirstChild(ps, "textStyle");
            if (ts == null) {
                ts = styDoc.createElement("textStyle");
                ps.appendChild(ts);
            }
            if (getFirstChild(ts, "lineOverflow") == null) {
                addText(styDoc, ts, "lineOverflow", "true");
            } else {
                setChildText(ts, "lineOverflow", "true");
            }
            cnt++;
        }
        return cnt;
    ...
    private void applyStyleOverride(Document doc, Element e, String name) {
        java.util.Map<String, java.util.Map<String, String>> ov = STYLE_OVERRIDE_MAP.get(name);
        if (ov == null)
            return;
        for (java.util.Map.Entry<String, java.util.Map<String, String>> te : ov.entrySet()) {
            Element t = getFirstChild(e, te.getKey());
            if (t == null)
                continue;
            for (java.util.Map.Entry<String, String> pe : te.getValue().entrySet()) {
                Element p = getFirstChild(t, pe.getKey());
                if (p != null)
    ...
    private int[] applyPatternA() {
        ...
        for (int i = 0; i < lcs.getLength(); i++) {
            ...
            for (Map.Entry<String, List<LayerEntry>> me : bkt.entrySet()) {
                ...
                for (LayerEntry e : sorted)
                    if (seen.add(e.styleName))
                        orderedList.add(e.styleName);
        ...
        for (int i = 0; i < lcs.getLength(); i++) {
            ...
            for (Element li : getDirectChildren(lc, "layerInfo")) {
                ...
                if (excl2.contains(e.styleName))
                    continue;
                bkt.computeIfAbsent(layerName(e), k -> new ArrayList<>()).add(e);
            }
            for (Map.Entry<String, List<LayerEntry>> me : bkt.entrySet()) {
                List<LayerEntry> entries = me.getValue();
                if (entries.size() <= 1)
                    continue;
                ...
                for (String tag : nonPt) {
                    ...
                    if (!created.contains(pname)) {
                        createElement(tag, pname, ordered, dRef);
                        created.add(pname);
                        createdCnt++;
                    }
                    for (LayerEntry e : te) {
                        lc.removeChild(e.layerInfoElem);
                        reducedCnt++;
                    }
    ...
    private int applyStrategyA() {
        ...
        for (Map.Entry<String, String> entry : STRATEGY_A.entrySet()) {
            ...
            for (int i = 0; i < lcs.getLength(); i++) {
                ...
                for (Element li : getDirectChildren(lc, "layerInfo")) {
                    LayerEntry e = makeEntry(li);
                    if (e == null || !e.baseLayerName.equals(base) || !"pointStyle".equals(e.styleTag))
                        continue;
        ...
        for (Map.Entry<String, String> entry : STRATEGY_C.entrySet()) {
            ...
            for (int i = 0; i < lcs.getLength(); i++) {
                ...
                for (Element li : getDirectChildren(lc, "layerInfo")) {
                    ...
                    if (e == null || !e.baseLayerName.equals(base) || !"pointStyle".equals(e.styleTag))
                        continue;
                    List<LeafQuery> lqs = getLeafQueries(e.styleName);
                    if (lqs.isEmpty())
                        continue;
                    LeafQuery lq = lqs.get(0);
                    Element leafElem = styleMap.get(lq.leafStyle);
                    if (leafElem == null)
                        continue;
                    LvInfo li2 = new LvInfo();
                    li2.styleName = e.styleName;
                    li2.lq = lq;
                    li2.vh = elemToSettings(leafElem);
                    li2.li = li;
                    li2.draw = e.drawOrder;
                    li2.full = e.fullLayerName;
            ...
            for (Map.Entry<String, List<Map.Entry<String, LvInfo>>> vg : vhGrps.entrySet()) {
                ...
                List<String[]> cqlLeafs = new ArrayList<>();
                Set<String> seenCql = new HashSet<>();
                for (Map.Entry<String, LvInfo> x : lvis) {
                    LvInfo li2 = x.getValue();
                    String ct = li2.lq.cql != null ? li2.lq.cql : li2.lq.field + "=" + li2.lq.value;
                    if (seenCql.add(ct))
                        cqlLeafs.add(new String[] { ct, li2.lq.leafStyle });
                }
                createParentCql("pointStyle", pname, cqlLeafs, lvis.get(0).getValue().styleName);
                for (Map.Entry<String, LvInfo> x : lvis)
                    levelParent.put(x.getKey(), pname);
                created++;
            }
            for (int i = 0; i < lcs.getLength(); i++) {
                Element lc = (Element) lcs.item(i);
                String level = getChildText(lc, "levelId");
                if (!levelParent.containsKey(level))
                    continue;
                LvInfo li2 = lvInfo.get(level);
                if (li2 == null)
                    continue;
    ...
    private Set<String> getMapInfoPointRefs() {
        ...
        for (int i = 0; i < sl.getLength(); i++) {
            ...
            if (e != null && "pointStyle".equals(e.getTagName()))
                refs.add(n);
        }
        return refs;
    }
    ...
    private int simplifyOneQueryStyles(Set<String> excludePoints) {
        ...
        for (String t : STYLE_TAGS)
            all.addAll(getDirectChildElements(styDoc.getDocumentElement(), t));
        for (Element elem : all) {
            List<Element> qs = getDirectChildren(elem, "query");
            if (qs.size() != 1)
                continue;
            Element q = qs.get(0);
            ...
            if (sn == null)
                continue;
            Element leaf = styleMap.get(sn.getTextContent().trim());
            if (leaf == null)
                continue;
            // leaf가 또 query를 가지면 체인 깊이 > 1 → 건너뜀
            if (!getDirectChildren(leaf, "query").isEmpty())
                continue;

            // ① 기존 시각 요소 제거 (priority 포함)
            for (Node c = elem.getFirstChild(); c != null;) {
                Node nx = c.getNextSibling();
                if (c.getNodeType() == Node.ELEMENT_NODE && VISUAL_TAGS.contains(c.getNodeName()))
                    elem.removeChild(c);
                c = nx;
            }
            ...
            for (Node c = leaf.getFirstChild(); c != null; c = c.getNextSibling()) {
                ...
                if (cn.equals("name") || cn.equals("query") || cn.equals("priority"))
                    continue;
            ...
            Element cqlEl = getFirstChild(q, "cql"), fldEl = getFirstChild(q, "name"),
                    valEl = getFirstChild(q, "value");
            if (cqlEl == null && fldEl != null) {
                String ct = toCql(fldEl.getTextContent().trim(), valEl != null ? valEl.getTextContent().trim() : "");
                q.removeChild(fldEl);
                if (valEl != null)
                    q.removeChild(valEl);
                Element nc = styDoc.createElement("cql");
                nc.setTextContent(ct);
                q.insertBefore(nc, q.getFirstChild());
            }
    ...
    private int removeUnreferenced() {
        ...
        NodeList sl = mapDoc.getElementsByTagName("selectStyleName");
        for (int i = 0; i < sl.getLength(); i++)
            collectReachable(sl.item(i).getTextContent().trim(), reachable);
        int cnt = 0;
        Element root = styDoc.getDocumentElement();
        for (String t : STYLE_TAGS)
            for (Element e : getDirectChildElements(root, t))
                if (!reachable.contains(getChildText(e, "name"))) {
                    root.removeChild(e);
                    cnt++;
                }
        return cnt;
    ...
    private void collectReachable(String name, Set<String> r) {
        if (name == null || r.contains(name) || !styleMap.containsKey(name))
            return;
        r.add(name);
        for (Element q : getDirectChildren(styleMap.get(name), "query")) {
            Element sn = getFirstChild(q, "styleName");
            if (sn != null)
                collectReachable(sn.getTextContent().trim(), r);
        }
    }
    ...
    private int[] applyNamingRules() {
        ...
        for (Map.Entry<String, List<String>> me : settingsMap.entrySet()) {
            List<String> names = me.getValue();
            if (names.size() <= 1)
                continue;
            String settings = me.getKey(), newName = null;
            int rule = 0;
            if (isMeinstyleGroup(names)) {
                String prefix = names.get(0).replaceAll("\\+메인스타일\\+L\\d+$", "");
                newName = prefix + "+메인스타일";
                rule = 1;
                ...
                colorSeriesCnt.merge(ck, 1, Integer::sum);
                int idx = colorSeriesCnt.get(ck);
                newName = series + "_" + colorKr + (idx > 1 ? "_" + idx : "");
                rule = 3;
            } else if (isLevelRangeGroup(names)) {
                newName = buildMergedName(names);
                rule = 2;
            } else {
                r4++;
                continue;
            ...
            if (usedNewNames.contains(newName)) {
                r4++;
                continue;
            } // 충돌 → 보류
            ...
            for (int i = 1; i < names.size(); i++) {
                renameMap.put(names.get(i), newName);
                deleteSet.add(names.get(i));
            }
            saved += names.size() - 1;
            if (rule == 1)
                r1++;
        ...
        for (String t : STYLE_TAGS) {
            ...
            for (Element e : getDirectChildElements(root, t)) {
                ...
                if (deleteSet.contains(old))
                    toRm.add(e);
                else if (renameMap.containsKey(old))
                    setChildText(e, "name", renameMap.get(old));
        ...
        for (int i = 0; i < sl.getLength(); i++) {
            ...
            if (renameMap.containsKey(t))
                sl.item(i).setTextContent(renameMap.get(t));
        }
    ...
    private static boolean isContourGroup(List<String> names) {
        for (String n : names) {
            boolean ok = false;
            for (String s : CONTOUR_SERIES)
                if (n.contains(s)) {
                    ok = true;
                    break;
                }
            if (!ok || (!n.contains("+ctrln_hg+") && !n.contains("+con_value+")))
                return false;
        }
        return true;
    }
    ...
    private static String getContourSeries(List<String> names) {
        for (String s : CONTOUR_SERIES)
            if (names.stream().allMatch(n -> n.contains(s)))
                return s;
        return "등고선";
    }

    private static String extractColor(String s) {
        Matcher m = Pattern.compile("\"color\":\"([^\"]+)\"").matcher(s);
    ...
    private static boolean isLevelRangeGroup(List<String> names) {
        ...
        for (String n : names)
            bases.add(n.replaceAll("\\d+_\\d+레벨|\\d+레벨", "LVL"));
        if (bases.size() == 1 && bases.iterator().next().contains("LVL"))
            return true;
        for (String p : new String[] { "지하철노선", "지하철역명", "지하철노선명", "건물_용도별", "지하철노선_구분" })
            if (names.stream().allMatch(n -> n.contains(p)))
                return true;
        return false;
    }
    ...
    private static String buildMergedName(List<String> names) {
        ...
        for (String n : names) {
            ...
            if (m.find()) {
                minL = Math.min(minL, Integer.parseInt(m.group(1)));
                maxL = Math.max(maxL, Integer.parseInt(m.group(2)));
            } else {
        ...
        if (minL == Integer.MAX_VALUE)
            return names.get(0);
        String ref = names.get(0);
        Matcher ml = Pattern.compile("(\\d+_\\d+레벨|\\d+레벨)").matcher(ref);
        if (!ml.find())
            return ref;
        String newLv;
        if (minL == maxL)
            newLv = minL + "레벨";
        else if (ref.contains("-") && ref.contains("레벨"))
            newLv = minL + "-" + maxL + "레벨";
        else if (ml.group(1).contains("_"))
            newLv = minL + "_" + maxL + "레벨";
        else if (ref.contains(" ") && ref.contains("레벨"))
            newLv = minL + "-" + maxL + "레벨";
        else
            newLv = minL + "_" + maxL + "레벨";
    ...
    private static void serializeElem(Element elem, StringBuilder sb) {
        ...
        for (Node c = elem.getFirstChild(); c != null; c = c.getNextSibling()) {
            ...
            boolean leaf = true;
            for (Node cc = ce.getFirstChild(); cc != null; cc = cc.getNextSibling())
                if (cc.getNodeType() == Node.ELEMENT_NODE) {
                    leaf = false;
                    break;
                }
            if (leaf)
                val.append('"').append(ce.getTextContent().trim()).append('"');
            else
                serializeElem(ce, val);
        ...
        for (Map.Entry<String, List<String>> me : cm.entrySet()) {
            ...
            if (me.getValue().size() == 1)
                sb.append(me.getValue().get(0));
            else
                sb.append('[').append(String.join(",", me.getValue())).append(']');
        }
        sb.append('}');
    }
    ...
    private int[] optimizeQueries() {
        ...
        for (String tag : STYLE_TAGS) {
            for (Element elem : getDirectChildElements(styDoc.getDocumentElement(), tag)) {
                ...
                for (Map.Entry<String, List<Object[]>> e : groups.entrySet()) {
                    ...
                    for (Object[] it : items) {
                        vals.add((String) it[0]);
                        elem.removeChild((Element) it[1]);
                    }
                    boolean allInt = vals.stream().allMatch(v -> {
                        try {
                            Integer.parseInt(v);
                            return true;
                        } catch (Exception ex) {
                    ...
                    addText(styDoc, nq, "cql", ct);
                    addText(styDoc, nq, "styleName", leaf);
                    elem.appendChild(nq);
                    inSaved += items.size() - 1;
                    merged = true;
                }
                if (merged) {
                    List<Element> allQs = getDirectChildren(elem, "query");
                    for (int i = 0; i < allQs.size(); i++) {
                        Element p = getFirstChild(allQs.get(i), "priority");
                        if (p != null)
                            p.setTextContent(String.valueOf(i + 1));
                        else
                            addText(styDoc, allQs.get(i), "priority", String.valueOf(i + 1));
                    }
                }
            }
        }
        ...
        for (String tag : STYLE_TAGS)
            for (Element elem : getDirectChildElements(styDoc.getDocumentElement(), tag))
                for (Element q : getDirectChildren(elem, "query")) {
                    Element fld = getFirstChild(q, "name"), val = getFirstChild(q, "value"),
                            cql = getFirstChild(q, "cql");
                    if (fld == null || cql != null)
                        continue;
        ...
        for (String tag : STYLE_TAGS)
            for (Element elem : getDirectChildElements(styDoc.getDocumentElement(), tag))
                for (Element q : getDirectChildren(elem, "query")) {
                    Element cqlEl = getFirstChild(q, "cql");
                    if (cqlEl == null)
                        continue;
                    Matcher m = inPat.matcher(cqlEl.getTextContent().trim());
                    if (!m.find())
                        continue;
                    ...
                    for (String v : m.group(2).split(",")) {
                        try {
                            nums.add(Integer.parseInt(v.trim()));
                        } catch (NumberFormatException ex) {
                            allInt = false;
                            break;
                        }
                    }
                    if (!allInt || nums.size() < 3)
                        continue;
                    ...
                    if (consec) {
                        cqlEl.setTextContent(
                                m.group(1) + " BETWEEN " + nums.get(0) + " AND " + nums.get(nums.size() - 1));
                        betweenConverted++;
                    }
                }
    ...
    private void createElement(String tag, String pname, List<String> childStyles, boolean directRef) {
        ...
        if (tag.equals("pointStyle"))
            addText(styDoc, parent, "priority", "0");
        ...
        if (directRef) {
            for (int i = 0; i < childStyles.size(); i++) {
                ...
                if (!isPointParent && getStyleTag(cn) != null && getStyleTag(cn).equals("pointStyle"))
                    continue;
                ...
                if (childElem != null) {
                    ...
                    if (!childQs.isEmpty()) {
                        ...
                        if (snEl != null) {
                            ...
                            if (leafElem != null) {
                                for (Node lc2 = leafElem.getFirstChild(); lc2 != null; lc2 = lc2.getNextSibling()) {
                                    if (lc2.getNodeType() != Node.ELEMENT_NODE)
                                        continue;
                                    String lTag = lc2.getNodeName();
                                    if (lTag.equals("name") || lTag.equals("query") || lTag.equals("priority"))
                                        continue;
                        ...
                        for (Element cqr : new ArrayList<>(childQs))
                            childElem.removeChild(cqr);
                        Element childPri = getFirstChild(childElem, "priority");
                        if (childPri != null)
                            childElem.removeChild(childPri);
                        // ④ 개별 속성 오버라이드 적용
            ...
            if (tag.equals("pointStyle"))
                addText(styDoc, parent, "transparency", "0");
            styDoc.getDocumentElement().appendChild(parent);
            styleMap.put(pname, parent);
            return;
        }
        ...
        for (String s : childStyles) {
            if (!isPointParent && getStyleTag(s) != null && getStyleTag(s).equals("pointStyle"))
                continue;
        ...
        for (int idx = 0; idx < total; idx++) {
            ...
            Element q = styDoc.createElement("query");
            if (lq.cql != null)
                addText(styDoc, q, "cql", lq.cql);
            else if (lq.field != null) {
                addText(styDoc, q, "name", lq.field);
                addText(styDoc, q, "value", lq.value);
            }
            if (lq.leafStyle != null)
                addText(styDoc, q, "styleName", lq.leafStyle);
    ...
    private void createParentCql(String tag, String pname, List<String[]> cqlLeafs, String baseStyle) {
        ...
        if (base != null)
            for (Node c = base.getFirstChild(); c != null; c = c.getNextSibling()) {
                if (c.getNodeType() != Node.ELEMENT_NODE)
                    continue;
                String cn = c.getNodeName();
                if (cn.equals("name") || cn.equals("query") || cn.equals("priority") || cn.equals("transparency"))
                    continue;
        ...
        for (String[] cl : cqlLeafs) {
            if (!seenCql.add(cl[0]))
                continue;
            Element q = styDoc.createElement("query");
            addText(styDoc, q, "cql", cl[0]);
            addText(styDoc, q, "styleName", cl[1]);
            addText(styDoc, q, "priority", String.valueOf(pri++));
            parent.appendChild(q);
        }
        if (tag.equals("pointStyle"))
    ...
    private List<LeafQuery> getLeafQueries(String styleName) {
        Element elem = styleMap.get(styleName);
        if (elem == null)
        ...
        for (Element q : qList) {
            ...
            if (snEl == null)
                continue;
            String leafName = snEl.getTextContent().trim();
            Element leafElem = styleMap.get(leafName);
            if (leafElem != null && !getDirectChildren(leafElem, "query").isEmpty()) {
                for (Element lq : getDirectChildren(leafElem, "query")) {
                    Element lsn = getFirstChild(lq, "styleName");
                    if (lsn == null)
                        continue;
                    LeafQuery o = new LeafQuery();
                    Element lcql = getFirstChild(lq, "cql"), lfld = getFirstChild(lq, "name"),
                            lval = getFirstChild(lq, "value");
                    o.cql = lcql != null ? lcql.getTextContent().trim()
                            : cqlEl != null ? cqlEl.getTextContent().trim() : null;
                    o.field = lfld != null ? lfld.getTextContent().trim() : null;
                    o.value = lval != null ? lval.getTextContent().trim() : null;
                    o.leafStyle = lsn.getTextContent().trim();
                    res.add(o);
                }
            } else {
                LeafQuery o = new LeafQuery();
    ...
    private LayerEntry makeEntry(Element li) {
        ...
        if (full == null || sn == null || draw == null)
            return null;
        String layer = full.contains(":") ? full.split(":")[1] : full;
        LayerEntry e = new LayerEntry();
        e.fullLayerName = full;
        e.baseLayerName = layer.replaceAll("_\\d+$", "");
        e.styleName = sn;
        e.drawOrder = Double.parseDouble(draw);
        e.layerInfoElem = li;
        e.styleTag = getStyleTag(sn);
        return e;
    }

    private String layerName(LayerEntry e) {
        return e.fullLayerName.contains(":") ? e.fullLayerName.split(":")[1] : e.fullLayerName;
    }

    private String getStyleTag(String name) {
        Element e = styleMap.get(name);
        return e != null ? e.getTagName() : null;
    }

    private static String toCql(String field, String value) {
    ...
    private static String levelSuffix(List<String> levels) {
        ...
        for (String lv : levels) {
            int v = Integer.parseInt(lv.trim());
            if (v < mn)
                mn = v;
            if (v > mx)
                mx = v;
    ...
    private void addLayerInfo(Element lc, String layerName, String styleName, String drawOrder) {
        Element li = mapDoc.createElement("layerInfo");
        addText(mapDoc, li, "layerName", layerName);
        addText(mapDoc, li, "selectStyleName", styleName);
        addText(mapDoc, li, "drawOrder", drawOrder);
        lc.appendChild(li);
    }
    ...
    private static List<Element> getDirectChildren(Element parent, String tag) {
        return getDirectChildElements(parent, tag);
    }

    private static Element getFirstChild(Element parent, String tag) {
        List<Element> l = getDirectChildren(parent, tag);
        return l.isEmpty() ? null : l.get(0);
    }

    private static String getChildText(Element parent, String tag) {
        Element c = getFirstChild(parent, tag);
        return c != null ? c.getTextContent().trim() : null;
    }

    private static void setChildText(Element parent, String tag, String text) {
        Element c = getFirstChild(parent, tag);
        if (c != null)
    ...
    // _view suffix 레이어 및 연관 스타일 제거
    private void removeViewLayers() {
        Set<String> viewStyles = new LinkedHashSet<>();
        NodeList lcs = mapDoc.getElementsByTagName("levelConfig");
        for (int i = 0; i < lcs.getLength(); i++) {
            Element lc = (Element) lcs.item(i);
            for (Element li : new ArrayList<>(getDirectChildren(lc, "layerInfo"))) {
                String ln = getChildText(li, "layerName");
                if (ln == null)
                    continue;
                String layer = ln.contains(":") ? ln.substring(ln.lastIndexOf(':') + 1) : ln;
                if (layer.endsWith("_view")) {
                    String sn = getChildText(li, "selectStyleName");
                    if (sn != null)
                        viewStyles.add(sn);
                    lc.removeChild(li);
                }
        ...
        for (String sn : viewStyles)
            collectChain(sn, toDelete);
        for (String sn : toDelete) {
            Element e = styleMap.get(sn);
            if (e != null && e.getParentNode() != null) {
                e.getParentNode().removeChild(e);
                styleMap.remove(sn);
            }
        }
        if (!viewStyles.isEmpty())
            out.printf("[_view삭제] mapInfo=%d건 / 스타일=%d개%n", viewStyles.size(), toDelete.size());
    }

    private void collectChain(String sname, Set<String> visited) {
        if (!visited.add(sname))
            return;
        Element e = styleMap.get(sname);
        if (e == null)
            return;
        for (Element q : getDirectChildren(e, "query")) {
            Element sn = getFirstChild(q, "styleName");
            if (sn != null)
                collectChain(sn.getTextContent().trim(), visited);
        }
    }
    ...
    // ══════════════════════════════════════════════════════════
    // STEP 마지막: mapInfo selectStyleName 동기화
    // styles.xml 에서 ~ → - 로 변환된 스타일명과 일치하도록
    // mapInfo.xml의 selectStyleName 내 ~ → - 변환
    // ══════════════════════════════════════════════════════════
    // ══════════════════════════════════════════════════════════
    // symbolStyle fieldName 오버라이드 (처리 완료 후 적용)
    // 고속도로번호+3+14+1 → 고속도로_{lb_name_k}.png
    // ══════════════════════════════════════════════════════════
    private int applySymbolFieldNameOverrides() {
        // {스타일명: 새 fieldName} 매핑
        java.util.Map<String, String> overrides = new LinkedHashMap<>();
        overrides.put("\uace0\uc18d\ub3c4\ub85c\ubc88\ud638+3+14+1",
                "\uace0\uc18d\ub3c4\ub85c_{lb_name_k}.png");

        int cnt = 0;
        for (String tag : new String[] { "pointStyle", "polygonStyle", "polylineStyle" }) {
            NodeList list = styDoc.getElementsByTagName(tag);
            for (int i = 0; i < list.getLength(); i++) {
                ...
                if (!overrides.containsKey(name))
                    continue;
                // symbolStyle → fieldName 수정
                NodeList symList = e.getElementsByTagName("symbolStyle");
                if (symList.getLength() == 0)
                    continue;
                Element sym = (Element) symList.item(0);
                NodeList fnList = sym.getElementsByTagName("fieldName");
                if (fnList.getLength() > 0) {
                    fnList.item(0).setTextContent(overrides.get(name));
                    cnt++;
                }
            }
        }
    ...
    // 지하철노선명(11_12/13_14레벨) 라벨은 겹칠 때 항상 뒤로 밀려서 "남는 공간에만 표시"되도록,
    // mapDoc 전체에서 관측되는 최대 drawOrder보다 큰 값으로 고정한다.
    private static final java.util.Set<String> SUBWAY_LINE_NAME_LAST_STYLES = new java.util.HashSet<>(
            java.util.Arrays.asList(
                    "tn_ex_ssubwln_l+지하철노선명" + "11_12레벨",
                    "tn_ex_ssubwln_l+지하철노선명" + "13_14레벨"));

    private int fixSubwayLineNameDrawOrderLast() {
        NodeList allLayerInfos = mapDoc.getElementsByTagName("layerInfo");

        double maxDrawOrder = 0;
        for (int i = 0; i < allLayerInfos.getLength(); i++) {
            Element li = (Element) allLayerInfos.item(i);
            String doStr = getChildText(li, "drawOrder");
            if (doStr == null)
                continue;
            try {
                double v = Double.parseDouble(doStr);
                if (v > maxDrawOrder)
                    maxDrawOrder = v;
            } catch (NumberFormatException ignore) {
            }
        }
        double fixedDrawOrder = maxDrawOrder + 1;

        int cnt = 0;
        for (int i = 0; i < allLayerInfos.getLength(); i++) {
            Element li = (Element) allLayerInfos.item(i);
            String sn = getChildText(li, "selectStyleName");
            if (sn == null || !SUBWAY_LINE_NAME_LAST_STYLES.contains(sn))
                continue;
            setChildText(li, "drawOrder", String.valueOf(fixedDrawOrder));
            cnt++;
        }
        return cnt;
    }

    private int syncSelectStyleNames() {
        int cnt = 0;
        NodeList nodes = mapDoc.getElementsByTagName("selectStyleName");
        for (int i = 0; i < nodes.getLength(); i++) {
            org.w3c.dom.Node node = nodes.item(i);
            String text = node.getTextContent();
            if (text.contains("~")) {
