...
public class StyleFactory {
				...
				for (Query query : querys) {
					...
					if (query.getCql() != null && query.getCql().length() > 0) {
						...
						if (query.getName() == null || query.getValue() == null) {
							// System.out.println("error, query name = " + query.getName() +", value = " +
							// query.getValue() +", styleName = " + query.getStyleName());
							// continue;
						} else {
					...
					if (subBse == null && query.getPriority() == -1) {
						System.out.println("PointStyle name=" + ps.getName() + ", subInfo name = " + query.getName()
								+ ", value = " + query.getValue() + ", style = "
								+ query.getStyleName());
						subBse = bse;
					} else {
						// System.out.println(subBse + " ===== " + query.getName() + "====" +
						// query.getValue() + "====" + query.getPriority());
						...
						if (createMode) {
							...
							for (com.gis.protocol.freegis3.PointStyle subTemp : pss) {
								if (subps.getName().equals(subTemp.getName())) {
									...
									if (query.getOverlapConfig() != null
											&& query.getOverlapConfig().getExtensionArea() != null) {
									...
									if (query.getOverlapConfig() != null
											&& query.getOverlapConfig().getDeleteOverlapMode() != null) {
						...
						/*
						 * com.gis.map.style.PointStyle baseClone = (com.gis.map.style.PointStyle)
						 * bse.clone();
						 * 
						 * if (query.getPriority() == -1) {
						 * baseClone.setCompare(subBse);
						 * subBse = baseClone;
						 * } else if (query.getStyleName() != null &&
						 * query.getStyleName().trim().length() > 0) {
						 * baseClone.setCompare(subBse);
		...
		for (PolygonStyle ps : pls) {
			...
			if (querys != null && querys.size() > 0) {
				...
				for (Query query : querys) {
					...
					BasicStyleExtend subBse = (BasicStyleExtend) nStyles.get(query.getStyleName());
					if (subBse == null && query.getPriority() == -1) {
						System.out.println("PolygonStyle name=" + ps.getName() + ", subInfo name = " + query.getName()
								+ ", value = " + query.getValue() + ", style = "
								+ query.getStyleName());
						subBse = bse;
					} else {
		...
		for (PolylineStyle ps : pgs) {
			...
			if (querys != null && querys.size() > 0) {
				...
				for (Query query : querys) {
					...
					BasicStyleExtend subBse = (BasicStyleExtend) nStyles.get(query.getStyleName());
					if (subBse == null && query.getPriority() == -1) {
						System.out.println("PolylineStyle name=" + ps.getName() + ", subInfo name = " + query.getName()
								+ ", value = " + query.getValue() + ", style = "
								+ query.getStyleName());
						subBse = bse;
					} else {
	...
	static public BasicStyleExtend createPointStyle(com.gis.protocol.freegis3.PointStyle ps_,
			com.gis.protocol.freegis3.Styles style) {
		...
		if (ps_.getSymbolStyle() != null) {
			...
			if (ss.getSymbolName() != null) {
				...
				BufferedImage bi = StorageMng
						.getSymbol((style.getSymbolPath() != null ? style.getSymbolPath() + File.separator : "")
								+ ss.getSymbolName());
				if (bi != null) {
		...
		if (ps_.getTextStyle() != null) {
			...
			Font font = StorageMng.getFont(ps.fontName, ps.fontType, ps.getFontSize(), ps.tracking, ps.ligature,
					ps.kerning, ps.widthRatio);
			if (font == null) {
			...
			if (ls != null && ls.getWidth() > 0) {
				...
				setLineStyle2(ps, ls);
				ps.outLineFont = StorageMng.getFont(ps.fontName, ps.fontType,
						ps.getFontSize() + ls.getWidth() * getHdScale(),
		...
		// ps.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,
		// ps_.getTransparency() * 0.01f));
	...
	static public void setLineStyle2(BasicStyleExtend bse, LineStyle fs) {
		...
		if (fs.getTransparency() > 0) {
			...
			// bse.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,(100-fs.getTransparency())
			// * 0.01f));
			// bse.setRenderingComposite(true);
		}
	...
	static public BasicStyleExtend createPolygonStyle(com.gis.protocol.freegis3.PolygonStyle ps,
			com.gis.protocol.freegis3.Styles style) {
	...
	static public BasicStyleExtend createPolylineStyle(com.gis.protocol.freegis3.PolylineStyle ps,
			com.gis.protocol.freegis3.Styles style) {
		BasicStyleExtend bs = new BasicStyleExtend();
		bs.setName(ps.getName());
		bs.setDesc(ps.getDesc());
		...
		if (ps.getLineStyle() != null && ps.getLineStyle().getCap() != null) {
			...
			// System.out.println("createPolylineStyle="+ ps.getName()+", ps
			// cap="+ps.getLineStyle().getCap().toString());
