...
package com.gis2.map.render.java2D;
...
public class Java2DConverter {
	...
	private GeneralPath toShape(MultiPolygon mls, int gridSize) throws NoninvertibleTransformException {
		GeneralPath path = new GeneralPath();

		for (int i = 0; i < mls.getNumGeometries(); i++) {
			Polygon lineString = (Polygon) mls.getGeometryN(i);
			path.append(toShape(lineString, gridSize), false);
		}
	...
	private GeneralPath toShape(Polygon pg, int gridSize) throws NoninvertibleTransformException {
		GeneralPath shape = new GeneralPath();
		
		for (int j = 0; j < pg.getNumInteriorRing(); j++) {
			
			LineString ring = pg.getInteriorRingN(j);
			
			Coordinate[] cds = ring.getCoordinates();
			
			boolean exceptionPoint = false;
			
			boolean previousException = true;
			
			for(int i=0; i<cds.length-1; i++) {
				Coordinate one = cds[i];
				Coordinate two = cds[i+1];
				
				Point2D oneView = toViewPoint(one);
				Point2D twoView = toViewPoint(two);
				...
				if(exceptionPoint == false && previousException == false) {
					//shape.moveTo((float) oneView.getX(), (float) oneView.getY());
					shape.lineTo((float) twoView.getX(), (float) twoView.getY());
				}
				else if(exceptionPoint == false && previousException == true) {
					shape.moveTo((float) oneView.getX(), (float) oneView.getY());
					shape.lineTo((float) twoView.getX(), (float) twoView.getY());
				}
				
				previousException = exceptionPoint;
			}
		}

		Coordinate[] cds = pg.getExteriorRing().getCoordinates();
	...
	private GeneralPath toShape(LineString lineString) throws NoninvertibleTransformException {
		...
		// System.out.println(viewPoint.getX() +":" + viewPoint.getY());
	...
	private Shape toShape(MultiPoint point) throws NoninvertibleTransformException {
		Rectangle2D.Double pointMarker = new Rectangle2D.Double(0.0, 0.0, POINT_MARKER_SIZE, POINT_MARKER_SIZE);
		Point2D viewPoint = toViewPoint(point.getCoordinate());
		pointMarker.x = (double) (viewPoint.getX() - (POINT_MARKER_SIZE / 2));
		pointMarker.y = (double) (viewPoint.getY() - (POINT_MARKER_SIZE / 2));
	...
	public Shape toShape(Geometry geometry, int gridSize) throws NoninvertibleTransformException {
		if (geometry.isEmpty()) {
			return new GeneralPath();
		}

		if (geometry instanceof Polygon) {
			return toShape((Polygon) geometry, gridSize);
		}

		if (geometry instanceof MultiPolygon) {
			return toShape((MultiPolygon) geometry, gridSize);
		}
	 ...
	 * example, if you pass in a GeometryCollection containing a ring and a disk,
	 * you cannot render them as such: if you use Graphics.fill, you'll get two
	 * disks, and if you use Graphics.draw, you'll get two rings. Solution: create
	 * Shapes for each element.
	 */
	...
	public Shape toShapeGrid(Geometry geometry, int gridSize) throws NoninvertibleTransformException {
		if (geometry instanceof Polygon) {
			return toShape((Polygon) geometry, gridSize);
		}
