...
package com.data.org.apache.jdbm;
...
public class DBStore
    ...
    private <A> long insert2(A obj, Serializer<A> serializer, DataInputOutput buf)
        ...
        //System.out.println("umcompress="+recid);
    ...
    private <A> A fetch2(long recid, final Serializer<A> serializer, final DataInputOutput buf)
        ...
        if(physLocation == PREALOCATE_PHYS_RECID){
        	...
        	throw new IOException("cache should prevent this!");
        }
    ...
    public long countRecords() throws IOException {
        ...
        while (page != 0) {
            ...
            System.out.println("PageIo="+io.getPageId());
            for (int i = 0; i < _logicMgr.ELEMS_PER_PAGE; i += 1) {
                ...
                //long loyroid = this._logicMgr.fetch(physRowId);
                
                //this._physMgr.fetch(out, rowid);
                
                

                if (physRowId != 0){
                    counter += 1;
                    System.out.println("p="+physRowId+", l=");
                }
            }
            _file.release(io);
    ...
    public List<Integer> print(){
    	...
    	try {
    		list = this._logicMgr.listRecords();
    		
    		for(Integer li : list){
    			System.out.println("temp="+this.decompressRecid(li.longValue()));
    		}
