...
final class LogicalRowIdManager {
    ...
    List<Integer> listRecords() throws IOException {
        int pos = Magic.DATA_PAGE_O_DATA;
        List<Integer> ret =new ArrayList<Integer>();
        for(
            long pageid = pageman.getFirst(Magic.USED_PAGE);pageid!=0;pageid = pageman.getNext(pageid)){
        	//System.out.println("pageid = " + pageid);
            PageIo page = pageman.file.get(pageid);
            System.out.println("PageIo="+page.getPageId()+",pageid = " + pageid);   
            ...
            while(pos < Storage.PAGE_SIZE -RecordHeader.SIZE){
                ...
                int currrSize =RecordHeader.getCurrentSize(page, (short) pos);
                pos+=size+RecordHeader.SIZE;
                if(currSize==0)
                    size = -size;
                
                System.out.println("listRecords PageIo="+page.getPageId()+",size = " + size+",recid=");   
                ret.add(size);
            }
